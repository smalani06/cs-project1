...
3. Command to ensure `Java/` is not in the new `Javaless_Docs.tar` archive: tar -tf Javaless_Docs.tar
...
The -x option is to extract files and directories from an archive while -c is used to create an archive. Using these options together will not work since they are opposing functions.

### Step 2: Create, Manage, and Automate Cron Jobs

1. Cron job for backing up the `/var/log/auth.log` file:

0 6 * * 3 root tar -czf /auth_backup.tgz /var/log/auth.log
